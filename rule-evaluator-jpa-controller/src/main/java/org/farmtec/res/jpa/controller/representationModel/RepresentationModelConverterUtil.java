...
public class RepresentationModelConverterUtil {
  ...
  public static EntityModel<PredicateLeaf> fromPredicateLeaf(PredicateLeaf p, long groupId) {
    return EntityModel.of(p,
        linkTo(methodOn(PredicateLeafController.class).getPredicateLeafById(p.getId()))
            .withSelfRel(),
        linkTo(methodOn(GroupCompositeController.class).deletePredicate(groupId, p.getId()))
            .withRel("deletePredicate"));
  }

  public static CollectionModel<EntityModel<PredicateLeaf>> fromPredicateLeaf(
      Set<PredicateLeaf> predicateLeaves, long groupId) {
    return CollectionModel.of(predicateLeaves.stream()
        .map(p -> RepresentationModelConverterUtil.fromPredicateLeaf(p, groupId))
        .collect(Collectors.toUnmodifiableSet()));
  }

  public static GroupCompositeRepresentationModel fromGroupComposite(GroupComposite groupComposite,
      long parentGroupId) {
    ...
    if (null != groupComposite.getPredicateLeaves()) {
      groupCompositeRepresentationModel.setPredicates(
          RepresentationModelConverterUtil
              .fromPredicateLeaf(groupComposite.getPredicateLeaves(), groupComposite.getId())
      );
    }
  ...
  public static RuleRepresentationModel ruleToRuleModel(Rule rule) {
    ...
    if (null != rule.getGroupComposite()) {
      ruleRepresentationModel.setGroup(RepresentationModelConverterUtil
          .fromGroupComposite(rule.getGroupComposite(), rule.getGroupComposite().getId()));
    }
    return ruleRepresentationModel;
  }

  private static CollectionModel<EntityModel<Action>> getActions(List<Action> actions, long ruleId) {
    return CollectionModel.of(actions.stream().map(action ->
        EntityModel.of(action,linkTo(methodOn(RuleController.class).deleteActionId(ruleId, action.getId()))
            .withRel("delete"))
    ).collect(Collectors.toUnmodifiableSet()));
  }
