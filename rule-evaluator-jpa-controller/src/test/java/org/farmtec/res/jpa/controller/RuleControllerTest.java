...
class RuleControllerTest {
  ...
  void setUp() {
    mockMvc = MockMvcBuilders
        .standaloneSetup(new RuleController(rulesRepository, new RulesValidatorImpl()))
    ...
    when(rulesRepository.findAll()).thenReturn(Arrays.asList(rule1, rule2));
  ...
  void getRuleById() throws Exception {
    mockMvc.perform(get("/rules/1"))
        ...
        .andExpect(jsonPath("$.group.groups.content[0].predicates.content[0].links[0].href",
            containsString("http://localhost/predicates/")))
        .andExpect(jsonPath("$.group.groups.content[0].predicates.content[0].links[1].href",
  ...
  @Test
  void deleteActionFromRule() throws Exception {
    assertThat(rule1.getActions().size()).isEqualTo(2);
    mockMvc.perform(delete("/rules/1/action/1"))
        .andExpect(status().isOk());
    assertThat(rule1.getActions().size()).isEqualTo(1);
  }

  @Test
  void deleteActionFromRule_whenRuleNotFound() throws Exception {

    when(rulesRepository.findById(anyLong())).thenReturn(Optional.empty());
    mockMvc.perform(delete("/rules/1/action/1"))
        .andDo(print())
  ...
  @Test
  void addActionToRule() throws Exception {

    //given
    Action action = new Action();
    action.setType("LIGHT");
    action.setData("Blinky");
    action.setPriority(3);
    ObjectMapper mapper = new ObjectMapper();

    String content = mapper.writeValueAsString(action);
    System.out.println(">>>>"+ content);
    int currentRuleActionCnt = rule1.getActions().size();
    //when
    mockMvc.perform(put("/rules/1/action/")
        .contentType(MediaType.APPLICATION_JSON)
        .content(content))
        .andDo(print())
        .andExpect(status().isOk());

    assertThat(rule1.getActions().size()).isEqualTo(currentRuleActionCnt +1);
  }

  @Test
  @Disabled
  void addActionToRule_whenRuleNotFound() throws Exception {
    when(rulesRepository.findById(anyLong())).thenReturn(Optional.empty());
    //given
    Action action = new Action();
    action.setType("LIGHT");
